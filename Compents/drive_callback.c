...
{
    uint8_t key = HAL_GPIO_ReadPin(pSteWhe->Key_GPIO_Port, pSteWhe->Key_GPIO_Pin);
    
    if (!(pSteWhe->ready_edge_flag & 0x20))
    {
        pSteWhe->ready_edge_flag |= 0x20;
        pSteWhe->ready_edge_flag = (pSteWhe->ready_edge_flag & ~0x10) | (key << 4);
    }
    
    if (key)
        pSteWhe->putoutDirection -= 0.3f;
    else
        pSteWhe->putoutDirection += 0.3f;

    if ((key << 4) != (pSteWhe->ready_edge_flag & 0x10))
    {
        pSteWhe->offset = pSteWhe->SteeringMotor.Angle;
        pSteWhe->putoutDirection = 0;
        pSteWhe->ready_edge_flag |= 0x80;
    }
